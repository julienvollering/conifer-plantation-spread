...
write_csv(lonlat, here("data", "raw", "NORA10", "poi-lonlat.csv"))
```
...
mutate(lonlat, SITE = 1:nrow(lonlat)) %>% 
  filter(SITE %in% NORAwind$SITE) %>% 
  write_csv(here("data","raw","NORA10","NORAsites.csv"))
```
...
EKLIMAwind <- lapply(EKLIMAwind, function(y) filter(y, FF != "x" & DD != "x")) %>% 
...
windstations %>%
  ...
  write_csv(here("data","raw","eKlima","EKLIMAsites.csv"))
```
