...
for (i in seq_along(datlist)) { # i <- 1
  ...
  m.wetland <- glmmTMB(formula = wildlings ~ seeds.WALD * age + bio01 + bio19 + 
    relelev + wetl + (1 | locality), data = dat, family = selmod$call$family, 
  ...
  m.disturb <- glmmTMB(formula = wildlings ~ seeds.WALD * age + bio01 + bio19 + 
                         relelev + nadisturb + andisturb + (1 | locality), 
                       data = dat, family = selmod$call$family, 
                       ziformula = selmod$call$ziformula)
...
saveRDS(results.dist, here("output","results-disturbance.rds"))
```
