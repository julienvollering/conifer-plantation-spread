...
for (j in species) { # j <- species[1]
  ...
  for (i in localities) { # i <- localities[1]
    ...
    grd <- grd %>% 
      mutate(seeds.ExP = seeds.ExP/sum(seeds.ExP, na.rm = TRUE), 
             seeds.WALD = seeds.WALD/sum(seeds.WALD, na.rm = TRUE))
...
localities <- read_csv(here("data", "localities.csv")) %>% 
  select(locality, species, utm33.easting, utm33.northing) %>% 
...
jointable <- raster::extract(chelsa, localities, sp = TRUE) %>% 
  ...
  rename(bio01 = CHELSA_bio_1, bio19 = CHELSA_bio_19)
anyNA(jointable)
```
