...
for (i in species) { # i <- species[1]
  ...
  for (j in localities) { # j <- localities[1]
    ...
    nsources <- st_area(source) %>% sum() %>% `/`(100) %>% drop_units() %>% round()
    set.seed(42)
    sources <- st_sample(source, nsources, type = "hexagonal") %>% st_sf()
...
```{r}
slow <- raster::raster(here("data","qc","Larix","anisdal","wald100daa.tif"))
fast <- raster::raster(here("data","qc","Larix","anisdal","wald.tif"))
st <- raster::stack(slow, fast)
raster::as.data.frame(st, na.rm=TRUE) %>% cor()
```
