...
```{r check heights, include=FALSE, eval=FALSE}
heightfiles <- list.files(here("data","qc"), pattern = "height.shp", recursive = TRUE)
hgts <- heightfiles %>% 
...
stack(hgts) %>% 
  ...
  arrange(ind, values)
stack(hgts) %>% 
  filter(ind=="P.sitchensis-lutzii/kvia" | ind=="P.sitchensis-lutzii/skorphella",
         values>30)

heightfiles %>% 
  set_names(gsub("/wildlings-height.shp","",.)) %>% 
  map(function(x) {
    pts <- st_read(here("data","qc",x), quiet = TRUE)
    if(has_name(pts, "hgt_assume")) {pull(pts, hgt_assume)} else {NA}
  })
```
...
excluded <- c("locality", "seeds.ExP", "seeds.WALD", "relelev")
...
percell.agg <- percell.agg %>% 
  ...
  select(species, locality, age, bio01, bio19, 
