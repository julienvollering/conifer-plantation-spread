...
```{r, eval=FALSE}
Pc_m1b <- update(Pc_m1, formula = update.formula(Pc_f1, ~ . - bio01 - bio19))
...
```{r}
ranef(Pc_m1)$cond
Pc_m1c <- update(Pc_m1, formula = update.formula(Pc_f1, ~ . - (1 | locality)))
# Warning messages:
# 1: In fitTMB(TMBStruc) :
#   Model convergence problem; non-positive-definite Hessian matrix. See vignette('troubleshooting')
# 2: In fitTMB(TMBStruc) :
...
```{r}
Pc_m1d <- update(Pc_m1, ziformula = ~ age)
summary(Pc_m1d)
```

```{r}
Pc_m2d <- update(Pc_m1d, formula = update.formula(Pc_f1, ~ . - seeds.WALD + seeds.ExP))
summary(Pc_m2d)
```

```{r, echo=FALSE}
bbmle::AICtab(Pc_m1d, Pc_m2d)
```
...
```{r}
simulationOutput <- simulateResiduals(fittedModel = Pc_m1d)
plot(simulationOutput)
testDispersion(simulationOutput)
testZeroInflation(simulationOutput)
```
...
models.list <- list(Ps_m1, Pa_m1b, L_m1, Pc_m1d)
