...
means <- tibble(species = NA_character_, age = NA_real_, bio01 = NA_real_,
                bio19 = NA_real_, seeds.ExP = NA_real_, seeds.WALD = NA_real_,
                relelev = NA_real_)
sds <- means
contrast <- "T4"
...
Ps_f1 <- formula("wildlings ~ seeds.WALD * age + bio01 + bio19 + relelev + 
                 T45 + artificial + T44 + T32 + V9 + T34 + V12 + V1 + T13 + T2 + 
                 T40 + T1 + V10 + V2 + T27 + T17 + V3 + T6 + T31 + T16 + V13 + 
                 T29 + T12 + T24 + T21 + T36 + (1 | locality)")
...
```{r}
perfectlyseparated <- c("T40", "T18", "T30", "T27", "T2", "V2", "T13", "T42", 
                        "V12", "V10", "T41")
removed <- c(perfectlyseparated, excluded)
L_dat <- L_dat %>% 
  filter_at(removed, all_vars(. < 0.5)) %>% 
...
L_f1 <- formula("wildlings ~ seeds.WALD * age + bio01 + bio19 + relelev + 
                T45 + artificial + T44 + T32 + V9 + T34 + V1 + T1 + T17 + 
                (1 | locality)")
...
```{r}
perfectlyseparated <- c("T40", "T42", "T6", "T31")
removed <- c(perfectlyseparated, excluded)
Pa_dat <- Pa_dat %>% 
  filter_at(removed, all_vars(. < 0.5)) %>% 
  filter(!(is.na(seeds.ExP)), !(is.na(seeds.WALD))) %>% 
...
Pa_f1 <- formula("wildlings ~ seeds.WALD * age + bio01 + bio19 + relelev + 
                 T45 + artificial + T44 + T32 + V9 + V1 + T41 + T2 + T1 + V10 + 
                 V2 + V4 + V3 + (1 | locality)")
...
Pc_f1 <- formula("wildlings ~ seeds.WALD * age + bio01 + bio19 + relelev + 
                 artificial + T34 + V1 + T40 + T1 + T30 + V3 + V8 + (1 | locality)")
...
sds <- rbind(Ps_sds, L_sds, Pa_sds, Pc_sds) %>% 
  as_tibble() %>% 
  add_column(species = species, .before = "age")
models.list <- list(Ps_m5, L_m5, Pa_m6, Pc_m7)
```
...
```{r}
results <- list(arith.list, means, sds, models.list, link.list)
