...
paste(typenames[!(typenames %in% c(cs, contrasttype))], collapse = " + ")

mf <- formula("wildlings ~ age + bio01 + bio19 + relelev + 
              T44 + artificial + T45 + T32 + V9 + T34 + V12 + V1 + T13 + T41 + 
              T1 + V2 + T2 + T40 + V10 + T27 + T17 + T29 + T16 + T31 + V3 + T6 + 
              V13 + T12 + T24 + T21 + T36 + (1 | locality)")
...
m1 <- fit_genpoisWALDmodel(dat[[1]], paste_modelformula(dat[[1]], typenames))
# Error in optimHess(par.fixed, obj$fn, obj$gr) : 
#   gradient in optim evaluated to length 1 not 34
summary(dat[[1]])
m2 <- fit_genpoisWALDmodel(dat[[2]], paste_modelformula(dat[[2]], typenames))
m3 <- fit_genpoisWALDmodel(dat[[3]], paste_modelformula(dat[[3]], typenames))
list(m.genpoisWALD, m2, m3) %>% 
  map(broom.mixed::tidy) %>% 
  set_names(nm = c("mall", "m2", "m3")) %>% 
  bind_rows(.id = "model") %>% 
  pivot_wider(id_cols = all_of(c("effect", "component", "term")), 
              names_from = "model", values_from = "estimate") %>% 
  View()
