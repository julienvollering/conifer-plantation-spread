...
```{r}
perfectlyseparated <- c("T42", "T18", "T3", "T41", "T33", "V4")
removed <- c(perfectlyseparated, excluded)
Ps_dat <- Ps_dat %>% 
  filter_at(removed, all_vars(. < 0.5)) %>%
  filter(!(is.na(seeds.ExP)), !(is.na(seeds.WALD))) %>% 
  select(-c('species'))
...
Ps_f1 <- formula("wildlings ~ seeds.WALD * age + bio01 + bio19 + relelev + 
                 T45 + artificial + T44 + T32 + V9 + T34 + V12 + V1 + T13 + T1 + 
                 V2 + T2 + T40 + V10 + T27 + T17 + T29 + T16 + V8 + T31 + V3 + 
                 T6 + V13 + T12 + T24 + T21 + T36 + (1 | locality)")
...
gg <- ggplot(link.scaled, aes(reorder(nin, fit), fit)) + 
  ...
  geom_point()  + 
  geom_point(data = filter(Ps_arith.scaled, nin %in% link.scaled$nin), 
             col = "grey", pch = 1)
...
Ps_localities <- filter(resp.locality, !is.na(locality))
...
gg <- ggplot(resp.locality.scaled, aes(reorder(nin, fit), fit)) + 
  ...
  geom_point(data = filter(resp.locality.scaled, is.na(locality)))  + 
  geom_point(data = filter(Ps_arith.scaled, nin %in% resp.locality.scaled$nin), 
             color = "grey50",pch = 1)
gg + scale_y_log10() + coord_flip()
```
...
gg <- ggplot(link.scaled, aes(reorder(nin, fit), fit)) + 
  geom_linerange(mapping = aes(ymin = ci.l, ymax = ci.u)) + 
  geom_point()  + 
  geom_point(data = filter(L_arith.scaled, nin %in% link.scaled$nin), 
             col = "grey", pch = 1)
gg + scale_y_log10() + coord_flip()
```
...
L_localities <- filter(resp.locality, !is.na(locality))

gg <- ggplot(resp.locality.scaled, aes(reorder(nin, fit), fit)) + 
  geom_point(data = filter(resp.locality.scaled, !is.na(locality)), 
             mapping = aes(color = locality), cex = 0.5, show.legend = FALSE)  + 
  geom_point(data = filter(resp.locality.scaled, is.na(locality)))  + 
  geom_point(data = filter(L_arith.scaled, nin %in% resp.locality.scaled$nin), 
             color = "grey50",pch = 1)
gg + scale_y_log10() + coord_flip()
```
...
```{r}
Pa_m6 <- glmmTMB(Pa_f1, Pa_dat, family = "genpois", ziformula = ~ age + (1 | locality))
summary(Pa_m6)
```
...
```{r}
Pa_m7 <- glmmTMB(Pa_f1, Pa_dat, family = poisson, ziformula = ~ age + (1 | locality))
summary(Pa_m7)
```
...
gg <- ggplot(link.scaled, aes(reorder(nin, fit), fit)) + 
  geom_linerange(mapping = aes(ymin = ci.l, ymax = ci.u)) + 
  geom_point()  + 
  geom_point(data = filter(Pa_arith.scaled, nin %in% link.scaled$nin), 
             col = "grey", pch = 1)
gg + scale_y_log10() + coord_flip()
```
...
Pa_localities <- filter(resp.locality, !is.na(locality))

gg <- ggplot(resp.locality.scaled, aes(reorder(nin, fit), fit)) + 
  geom_point(data = filter(resp.locality.scaled, !is.na(locality)), 
             mapping = aes(color = locality), cex = 0.5, show.legend = FALSE)  + 
  geom_point(data = filter(resp.locality.scaled, is.na(locality)))  + 
  geom_point(data = filter(Pa_arith.scaled, nin %in% resp.locality.scaled$nin), 
             color = "grey50",pch = 1)
gg + scale_y_log10() + coord_flip()
```
...
Pc_dat <- Pc_dat %>% 
  filter_at(removed, all_vars(. < 0.5)) %>% 
  filter(!(is.na(seeds.ExP) | is.na(seeds.WALD) | is.na(relelev))) %>% 
  select(-c('species'))
...
#  gradient in optim evaluated to length 1 not 20
# Warning messages:
# 1: In (function (start, objective, gradient = NULL, hessian = NULL,  ... :
#   NA/NaN function evaluation
```

From glmmTMB troubleshooting vignette: "Try rescaling predictor variables. Try a simpler model and build up."

```{r}
Pc_m6 <- glmmTMB(Pc_f1, Pc_dat, family = "genpois", ziformula = ~ age + (1 | locality))
summary(Pc_m6)
```
...
```{r, echo=FALSE}
bbmle::AICtab(Pc_m3, Pc_m4, Pc_m6)
```

```{r}
simulationOutput <- simulateResiduals(fittedModel = Pc_m6)
plot(simulationOutput)
testDispersion(simulationOutput)
testZeroInflation(simulationOutput)
```
...
gg <- ggplot(link.scaled, aes(reorder(nin, fit), fit)) + 
  geom_linerange(mapping = aes(ymin = ci.l, ymax = ci.u)) + 
  geom_point()  + 
  geom_point(data = filter(Pc_arith.scaled, nin %in% link.scaled$nin), 
             col = "grey", pch = 1)
gg + scale_y_log10() + coord_flip()
```
...
Pc_localities <- filter(resp.locality, !is.na(locality))

gg <- ggplot(resp.locality.scaled, aes(reorder(nin, fit), fit)) + 
  geom_point(data = filter(resp.locality.scaled, !is.na(locality)), 
             mapping = aes(color = locality), cex = 0.5, show.legend = FALSE)  + 
  geom_point(data = filter(resp.locality.scaled, is.na(locality)))  + 
  geom_point(data = filter(Pc_arith.scaled, nin %in% resp.locality.scaled$nin), 
             color = "grey50",pch = 1)
gg + scale_y_log10() + coord_flip()
```
...
```{r}
species <- c("P.sitchensis-lutzii", "Larix", "P.abies", "P.contorta")

arith.list <- list(Ps_arith.scaled, L_arith.scaled, Pa_arith.scaled, Pc_arith.scaled)
names(arith.list) <- species

means <- rbind(Ps_means, L_means, Pa_means, Pc_means) %>% 
  as_tibble() %>% 
  add_column(species = species, .before = "age")
sds <- rbind(Ps_sds, L_sds, Pa_sds, Pc_sds) %>% 
  as_tibble() %>% 
  add_column(species = species, .before = "age")

models.list <- list(Ps_m5, L_m5, Pa_m6, Pc_m6)
names(models.list) <- species

link.list <- list(Ps_link, L_link, Pa_link, Pc_link) %>% 
  map(~ select(., locality, nin, fit, ci.l, ci.u))
names(link.list) <- species

locality.list <- list(Ps_localities, L_localities, Pa_localities, Pc_localities) %>% 
  map(~ select(., locality, nin, fit))
names(locality.list) <- species
...
results <- list(arithmetic = arith.list, 
                ...
                model.preds.localities = locality.list)
str(results, 2)
