...
models.list <- list(Ps_m5, Pa_m6, L_m5, Pc_m6)
names(models.list) <- species

link.list <- list(Ps_link, Pa_link, L_link, Pc_link) %>% 
  map(~ select(., locality, nin, fit, ci.l, ci.u))
names(link.list) <- species

locality.list <- list(Ps_localities, Pa_localities, L_localities, Pc_localities) %>% 
