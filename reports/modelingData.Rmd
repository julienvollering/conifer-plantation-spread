...
```{r}
L_arith <- pivot_longer(L_dat, 9:23, names_to = "nin", values_to = "are") %>%
  group_by(nin) %>% 
...
link.exp <- mutate(link, lower = fit - 3*se.fit) %>% 
  mutate(expfit = exp(fit), explower = exp(lower)) %>% 
  mutate(expse = expfit - explower)
