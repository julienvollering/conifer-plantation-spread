...
Pa_f1 <- formula("wildlings ~ seeds.WALD * age + bio01 + bio19 + relelev + 
                 cultivated + disturbed + T32 + fen + T2 + T1 + swamp + spring +
                 bog + (1 | locality)")
...
Overparameterized model? bio01 and bio19 show largest coefficients (5-6) while insignificant for other species. Try dropping them.
...
gg + scale_y_log10() + coord_flip() + 
  labs(x = 'NiN type', y = 'relative establishment probability', title = 'Relative establishment across species and NiN types', caption = 'filled points are model estimates with lines showing 95% confidence intervals, open points are calculated directly from the number of wildlings per unit area of each type')
```
