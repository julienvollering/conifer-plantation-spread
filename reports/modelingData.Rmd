...
```{r}
Ps_m2 <- update(Ps_m1, formula = update.formula(Ps_f1, ~ . - seeds.WALD + seeds.ExP - seeds.WALD:age + seeds.ExP:age))
summary(Ps_m2)
```
...
```{r}
Pa_m2 <- update(Pa_m1, formula = update.formula(Pa_f1, ~ . - seeds.WALD + seeds.ExP - seeds.WALD:age + seeds.ExP:age))
summary(Pa_m2)
```
...
```{r}
Pa_m1b <- update(Pa_m1, family = "genpois")
summary(Pa_m1b)
```

```{r}
simulationOutput <- simulateResiduals(fittedModel = Pa_m1b)
plot(simulationOutput)
testDispersion(simulationOutput)
testZeroInflation(simulationOutput)
```
...
```{r}
Pa_m1c <- update(Pa_m1, family = "poisson")
summary(Pa_m1c)
```

```{r}
simulationOutput <- simulateResiduals(fittedModel = Pa_m1c)
plot(simulationOutput)
testDispersion(simulationOutput)
testZeroInflation(simulationOutput)
```
...
```{r}
L_m2 <- update(L_m1, formula = update.formula(L_f1, ~ . - seeds.WALD + seeds.ExP - seeds.WALD:age + seeds.ExP:age))
summary(L_m2)
```
...
```{r}
ranef(Pc_m1)$cond
Pc_m1b <- update(Pc_m1, formula = update.formula(Pc_f1, ~ . - (1 | locality)))
# Warning messages:
# 1: In fitTMB(TMBStruc) :
#   Model convergence problem; non-positive-definite Hessian matrix. See vignette('troubleshooting')
# 2: In fitTMB(TMBStruc) :
...
The random effect variance of locality in the zero-inflation (binomial) model is very large. With only six localities, perhaps there is not enough data to estimate this parameter. Try dropping:

```{r}
Pc_m1c <- update(Pc_m1, ziformula = ~ age)
summary(Pc_m1c)
```

```{r}
Pc_m2c <- update(Pc_m1c, formula = update.formula(Pc_f1, ~ . - seeds.WALD + seeds.ExP - seeds.WALD:age + seeds.ExP:age))
summary(Pc_m2c)
```

```{r, echo=FALSE}
bbmle::AICtab(Pc_m1c, Pc_m2c)
```
...
```{r}
simulationOutput <- simulateResiduals(fittedModel = Pc_m1c)
plot(simulationOutput)
testDispersion(simulationOutput)
testZeroInflation(simulationOutput)
```
...
models.list <- list(Ps_m1, Pa_m1b, L_m1, Pc_m1c)
