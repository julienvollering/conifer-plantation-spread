...
DV %>% 
  ...
  group_split(species) %>% 
  map2(group_split(IV, species), ~ add_wildlings(.y,.x)) %>% 
  map(\(x) mutate(x, area = st_area(x)) %>% 
        st_drop_geometry()) %>% 
  map(\(x) select(x, species, wildlings, area)) %>% 
  map(\(x) summarize(x, species = first(species),
                     wildlings = sum(wildlings), 
                     area = sum(area))) %>% 
  list_rbind() %>% 
  mutate(area = units::set_units(area, "hectare")) %>% 
  mutate(density = wildlings/area)
```
