...
Ps_dat <- mutate(Ps_dat, across(starts_with("seeds"), ~ .x - mean(.x)))
...
Error in optimHess(par.fixed, obj$fn, obj$gr) : 
  gradient in optim evaluated to length 1 not 21

```{r, include=FALSE}
L_modelcomp <- bbmle::ICtab(L_m1, L_m2, base = TRUE, delta = TRUE, 
                            sort = FALSE) %>% 
  as_tibble() %>% 
...
Pc_dat <- mutate(Pc_dat, across(starts_with("seeds"), ~ .x - mean(.x)))
...
```{r}
Pc_f <- formula("wildlings ~ age + bio19 + relelev + 
                 artificial + T34 + V1 + T1 + T40 + T30 + V8 + V3 + (1 | locality)")
Pc_m1 <- glmmTMB(Pc_f, Pc_dat, offset = seeds.WALD, family = "genpois", 
                 ziformula = ~ age + (1 | locality))
summary(Pc_m1)
```

```{r}
Pc_m2 <- update(Pc_m1, offset = seeds.ExP)
summary(Pc_m2)
```
...
```{r}
Pc_m3 <- update(Pc_m1, offset = NULL)
summary(Pc_m3)
```
...
```{r}
simulationOutput <- simulateResiduals(fittedModel = Pc_m1)
plot(simulationOutput)
testDispersion(simulationOutput)
testZeroInflation(simulationOutput)
```
...
sensmod <- update(selmod, update.formula(formula(selmod), ~ . + seeds.WALD), offset = NULL)
newdat <- filter(newdat, is.na(locality))
Pc_link_sens <- mutate(newdat, fit = predict(sensmod, newdat, se.fit = FALSE, type = "conditional"))
scaler <- filter(Pc_link_sens, nin == "T4") %>% 
  pull(fit)
Pc_link_sens <- mutate(Pc_link_sens, fit = fit/scaler) %>% 
  select(nin, fit)
```
