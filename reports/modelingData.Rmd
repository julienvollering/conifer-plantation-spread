...
datlist <- datlist %>% 
  map(~ filter(., `plantation forest` < 0.5) %>% 
        select(-"plantation forest")
  )

nottypes <- c('species', 'locality','wildlings','age','bio01','bio19',
              'seeds.ExP','seeds.WALD','relelev')

contrast <- "T4"
str(datlist, 1)

map(datlist, function(x) {sum(x$wildlings) / nrow(x) * 100}) # wildlings per hectare
```
...
```{r}
typenames <- names(Ps_dat)[!names(Ps_dat) %in% nottypes]
...
```{r}
Ps_means <- Ps_dat[, c('age','bio01','bio19','relelev')] %>% map_dbl(mean)
Ps_sds <- Ps_dat[, c('age','bio01','bio19','relelev')] %>% map_dbl(sd)
for (i in c('age','bio01','bio19','relelev')) {
  Ps_dat <- modify_at(Ps_dat, .at = i, ~ (. - Ps_means[i]) / Ps_sds[i])
}
Ps_dat <- mutate(Ps_dat, across(starts_with("seeds"), log))
summary(Ps_dat)
...
Ps_m1 <- glmmTMB(Ps_f1, Ps_dat, family = "nbinom2", ziformula = ~ age + (1 | locality))
summary(Ps_m1)
```

```{r}
Ps_m2 <- update(Ps_m1, update.formula(Ps_f1, ~ . - seeds.WALD + seeds.ExP))
summary(Ps_m2)
```

```{r}
Ps_m3 <- update(Ps_m1, update.formula(Ps_f1, ~ . - seeds.WALD))
summary(Ps_m3)
```

```{r, include=FALSE}
Ps_modelcomp <- bbmle::ICtab(Ps_m1, Ps_m2, Ps_m3, base = TRUE, delta = TRUE, 
                             sort = FALSE) %>% 
  as_tibble() %>% 
...
```{r}
Ps_m1b <- update(Ps_m1, family = "genpois")
summary(Ps_m1b)
```

```{r}
simulationOutput <- simulateResiduals(fittedModel = Ps_m1b)
plot(simulationOutput)
testDispersion(simulationOutput)
testZeroInflation(simulationOutput)
```
...
```{r}
Ps_m1c <- update(Ps_m1, family = "poisson")
summary(Ps_m1c)
```

```{r}
simulationOutput <- simulateResiduals(fittedModel = Ps_m1c)
plot(simulationOutput)
testDispersion(simulationOutput)
testZeroInflation(simulationOutput)
```
...
```{r, echo=FALSE}
bbmle::AICtab(Ps_m1b, Ps_m1c)
```

Continue with generalized poisson model: diagnostics OK and best AIC. 

```{r, include=FALSE}
simulationOutput <- simulateResiduals(fittedModel = Ps_m2)
plot(simulationOutput)
testDispersion(simulationOutput)
testZeroInflation(simulationOutput)

simulationOutput <- simulateResiduals(fittedModel = Ps_m3)
plot(simulationOutput)
testDispersion(simulationOutput)
testZeroInflation(simulationOutput)

Ps_m3b <- update(Ps_m3, family = "genpois")
simulationOutput <- simulateResiduals(fittedModel = Ps_m3b)
plot(simulationOutput)
testDispersion(simulationOutput)
testZeroInflation(simulationOutput)

Ps_modelcomp <- bbmle::ICtab(Ps_m1b, Ps_m2, Ps_m3b, base = TRUE, delta = TRUE, 
                             sort = FALSE) %>% 
  as_tibble() %>% 
  add_column(`seed dispersal estimate` = c('WALD', 'ExP', 'none'), .before = 1)
```
...
newdat <- attributes(terms)$factors %>% 
  as_tibble() %>% 
  select(-locality) %>% 
  mutate(seeds.WALD = mean(selmod$frame$seeds.WALD), 
         age = 0, bio01 = 0, bio19 = 0, relelev = 0) %>%   
  distinct()
newdat <- newdat %>% 
...
```{r}
Pa_means <- Pa_dat[, c('age','bio01','bio19','relelev')] %>% map_dbl(mean)
...
Pa_m1 <- glmmTMB(Pa_f1, Pa_dat, family = "nbinom2", ziformula = ~ age + (1 | locality))
summary(Pa_m1)
```

```{r}
Pa_m2 <- update(Pa_m1, update.formula(Pa_f1, ~ . - seeds.WALD + seeds.ExP))
summary(Pa_m2)
```

```{r}
Pa_m3 <- update(Pa_m1, update.formula(Pa_f1, ~ . - seeds.WALD))
summary(Pa_m3)
```
...
L_f1 <- formula("wildlings ~ seeds.WALD + age + bio01 + bio19 + relelev + 
                ...
                T17 + (1 | locality)")
L_m1 <- glmmTMB(L_f1, L_dat, family = "nbinom2", ziformula = ~ age + (1 | locality))
summary(L_m1)
```

```{r}
L_m2 <- update(L_m1, update.formula(L_f1, ~ . - seeds.WALD + seeds.ExP))
summary(L_m2)
```

```{r}
L_m3 <- update(L_m1, update.formula(L_f1, ~ . - seeds.WALD))
summary(L_m3)
```
...
```{r}
typenames <- names(Pc_dat)[!names(Pc_dat) %in% nottypes]
...
```{r}
Pc_means <- Pc_dat[, c('age','bio01','bio19','relelev')] %>% map_dbl(mean)
Pc_sds <- Pc_dat[, c('age','bio01','bio19','relelev')] %>% map_dbl(sd)
for (i in c('age','bio01','bio19','relelev')) {
  Pc_dat <- modify_at(Pc_dat, .at = i, ~ (. - Pc_means[i]) / Pc_sds[i])
}
Pc_dat <- mutate(Pc_dat, across(starts_with("seeds"), log))
summary(Pc_dat)
...
Exclude bio01 due to very high correlation with bio19. 

### Compare WALD vs ExP vs null

```{r}
Pc_f1 <- formula("wildlings ~ seeds.WALD + age + bio19 + relelev + 
                 artificial + T34 + V1 + T1 + T40 + T30 + V8 + V3 + (1 | locality)")
Pc_m1 <- glmmTMB(Pc_f1, Pc_dat, family = "nbinom2", ziformula = ~ age + (1 | locality))
summary(Pc_m1)
```

```{r}
Pc_m2 <- update(Pc_m1, update.formula(Pc_f1, ~ . - seeds.WALD + seeds.ExP))
summary(Pc_m2)
```

```{r}
Pc_m3 <- update(Pc_m1, update.formula(Pc_f1, ~ . - seeds.WALD))
summary(Pc_m3)
```
...
```{r, include=FALSE}
Pc_modelcomp <- bbmle::ICtab(Pc_m1, Pc_m2, Pc_m3, base = TRUE, delta = TRUE, 
                             sort = FALSE) %>% 
  as_tibble() %>% 
  add_column(`seed dispersal estimate` = c('WALD', 'ExP', 'none'), .before = 1)
```
...
```{r}
simulationOutput <- simulateResiduals(fittedModel = Pc_m2)
plot(simulationOutput)
testDispersion(simulationOutput)
testZeroInflation(simulationOutput)
```
...
newdat <- attributes(terms)$factors %>% 
  as_tibble() %>% 
  select(-locality) %>% 
  mutate(seeds.ExP = mean(selmod$frame$seeds.ExP), 
         age = 0, bio01 = 0, bio19 = 0, relelev = 0) %>%   
  distinct()
newdat <- newdat %>% 
  mutate(nin = names(newdat)[apply(newdat, 1, match, x = 1)]) %>% 
...
```{r}
Pc_link.scaled %>% 
  filter(is.na(locality), nin %in% filter(Pc_arith.scaled, daa > 10, n > 1)$nin) %>% 
  pull(fit) %>% range() %>% log10() %>% diff()
```
...
models.list <- list(Ps_m1b, Pa_m1, L_m1, Pc_m2)
