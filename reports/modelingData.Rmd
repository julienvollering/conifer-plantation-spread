...
```{r}
Ps_m3 <- update(Ps_m1, formula = update.formula(Ps_f1, ~ . - seeds.WALD))
summary(Ps_m3)
```
...
```{r}
Pa_m3 <- update(Pa_m1, formula = update.formula(Pa_f1, ~ . - seeds.WALD))
summary(Pa_m3)
```
...
```{r}
Pa_m2b <- update(Pa_m2, family = "genpois")
Pa_m3b <- update(Pa_m3, family = "genpois")
Pa_modelcomp <- bbmle::ICtab(Pa_m1b, Pa_m2b, Pa_m3b, base = TRUE, delta = TRUE) %>% 
  as_tibble() %>% 
  add_column(`seed dispersal estimate` = c('WALD', 'ExP', 'none'), .before = 1)
```
...
```{r}
L_m3 <- update(L_m1, formula = update.formula(L_f1, ~ . - seeds.WALD))
summary(L_m3)
```
...
```{r}
Pc_m3 <- update(Pc_m1, formula = update.formula(Pc_f1, ~ . - seeds.WALD))
summary(Pc_m3)
```
...
models.list <- list(Ps_m1, Pa_m1b, L_m1, Pc_m1)
