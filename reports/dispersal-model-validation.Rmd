...
g <- st_join(grd, vegheight)
landscape <- st_join(pts, g)
plot(landscape)
```
...
for (i in seq_along(src$geometry)) { # i <- 50
  ...
  landscape <- mutate(landscape, angle = angle(dX, dY)) %>% # compass bearing FROM landscape TO source (matches wind directions)
  ...
  for (j in seq_along(levels(landscape$sector))) { # j <- 1
    ...
    wind.arr <- array(data = NA, dim = c(nx, ny, length(wind)), 
                 dimnames = list(x=seq(nx), y=seq(ny), src=seq(length(wind))))
    ...
    for (k in seq_along(wind)) { # k <- 1
      ...
      which(as_tibble(landscape)[,"sector"] == levels(landscape$sector)[j])
