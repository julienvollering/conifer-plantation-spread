...
# Dispersal model comparisons

```{r}
results.dispersalcomparison <- readRDS(here("output","results-dispersalcomparison.rds"))

modelcomp <- results.dispersalcomparison %>% 
  set_names(c("Ps", 
              "Pa", 
              "L")) %>% 
...
write.csv(modelcomp, here('ms', 'tables', 'dispmodelcomp.csv'), row.names = FALSE)
```
...
tidied <- results.models %>% 
  ...
  mutate(species = case_when(
    species == 'P.sitchensis-lutzii' ~ 'Ps',
    species == 'P.abies' ~ 'Pa',
    species == 'Larix' ~ 'L'
  )) %>% 
  mutate(speciesname = case_when(
    species == 'Ps' ~ 'Picea sitchensis / Picea \\times lutzii',
