...
log(\mu_{ij}) = SeedDeposition_{ij} \cdot PlantationAge_{ij} + Bio1_{ij} + Bio19_{ij} + RelativeElevation_{ij} + Locality_{i} + \sum\limits_{k=1}^K EcosystemType_{ijk} \\
...
For each species we dropped ecosystem types that completely free of wildlings (to avoid model convergence issues stemming from complete separation).
...
To achieve convergence we dropped locality from the binomial model, as its variance was likely difficult to estimate from only six localities.
...
From our first analysis, observed wildlings densities in different ecosystem types (excluding ecosystem types covering less than 1 ha) ranged 1-60/ha for Sitka spruce, 0-49/ha for Norway spruce, 0-52/ha for larch, and 0-219/ha for lodgepole pine.
The "open active landslide" type exceeded these densities around Sitka/Lutz spruce and larch plantations, but was restricted to very small areas.

Among ecosystem types tallying at least one wildling and covering at least 1 ha, modeled establishment probabilities varied by up to five orders of magnitude (fig. \@ref(fig:relative-establishment)).


```{r relative-establishment, fig.cap="Relative establishment probabilities among ecosystem types, using "forest" as the reference level, for four alien conifer species groups. Unfilled points show scaled wildling densities calculated directly from the data. In the case of zero wildlings, these points are placed at the lower limit of the x-axis. Filled points with line intervals show modeled estimates surrounded by their 95% confidence intervals. Grey points show modeled estimates for individual localities. Ecosystem types are ordered by a confidence-weighted mean of their percentile-ranked establishment probabilities within species, such that types with consistently high establishment probability are on top. Ecosystem types shown in italic font are wetlands, and ecosystem types shown in bold font are characterized by disturbance. Ecosystem types without wildlings and covering less than 0.5 ha across all localities for that species are not displayed. Photos licensed CC BY 4.0 Rune Halvorsen."}
knitr::include_graphics(here('ms', 'figures', 'relative-establishment', 'relative-establishment.pdf'))
```

For all species, estimates of seed deposition derived from the WALD model of seed dispersal fit patterns of wildling establishment better than estimates derived from the generalized seed dispersal kernel we tested.  


# Discussion

1. Obvious management recommendation: try to limit plantations to sites where they are surrounded by arable fields.  
2. Beware that Sitka colonization varies enormously from site to site --- much more than other species --- so our ability to predict the magnitude of spread at a particular site are limited. More research needed.  
...
Limitation:
Spatial grain. Minimum polygon area NiN, wildling group locations. A good reason to use a relatively course observation units (100 m2) and to perform fuzzy coding of ecosystem types. Did our best to represent ecotones in the models and not use polygon boundaries overconfidently.
...
Limitation:
Inland versus coastal P.contorta sites. Number of wildlings, climate...

Note that our models do not estimate the *total* causal effect of climate or relative elevation on wildlings.
