...
Wildling densities across all ecosystems ranged 0-211/ha for Sitka spruce, 0-49/ha for Norway spruce, 0-1045/ha for larches, and 0-219/ha for lodgepole pine.
Relative establishment likelihoods varied more than the corresponding set of densities, in all species (fig. \@ref(fig:establishment)).
...
kbl(msumms$`Ps-modelsummary`, longtable = TRUE, booktabs = TRUE, align = 'clrrrr', 
    caption = "\\label{tab:Ps}Model summary for Picea sitchensis / lutzii. The conditional submodel is glmmTMB's nbinom2 family with dispersion parameter phi = 0.244.") %>%
  kable_styling(font_size = 6) %>% 
  add_header_above(c(" " = 2, "Fixed effects" = 2, "Random effects" = 2)) %>% 
  column_spec(1, bold = T) %>% 
  collapse_rows(columns = 1, valign = "top")

kable(msumms$`Pa-modelsummary`, longtable = TRUE, booktabs = TRUE, align = 'clrrrr', caption = "\\label{tab:Pa}Model summary for Picea abies. The conditional submodel is glmmTMB's genpois family with dispersion parameter phi = 2.6.") %>%
  kable_styling(font_size = 6) %>% 
  add_header_above(c(" " = 2, "Fixed effects" = 2, "Random effects" = 2)) %>% 
  column_spec(1, bold = T) %>% 
  collapse_rows(columns = 1, valign = "top")

kable(msumms$`L-modelsummary`, longtable = TRUE, booktabs = TRUE, align = 'clrrrr', caption = "\\label{tab:L}Model summary for Larix. The conditional submodel is glmmTMB's nbinom2 family with dispersion parameter phi = 0.203.") %>%
  kable_styling(font_size = 6) %>% 
  add_header_above(c(" " = 2, "Fixed effects" = 2, "Random effects" = 2)) %>% 
  column_spec(1, bold = T) %>% 
  collapse_rows(columns = 1, valign = "top")

kable(msumms$`Pc-modelsummary`, longtable = TRUE, booktabs = TRUE, align = 'clrrrr', caption = "\\label{tab:Pc}Model summary for Pinus contorta. The conditional submodel is glmmTMB's nbinom2 family with dispersion parameter phi = 0.031.") %>%
  kable_styling(font_size = 6) %>% 
  add_header_above(c(" " = 2, "Fixed effects" = 2, "Random effects" = 2)) %>% 
  column_spec(1, bold = T) %>% 
  collapse_rows(columns = 1, valign = "top")
