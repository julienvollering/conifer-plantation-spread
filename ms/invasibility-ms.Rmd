...
Accordingly, we applied zero-inflated generalized linear models, which model zeros as the mixed product of a binomial process and a conditional count process.
...
For each species we dropped ecosystem types that did not coincide with a single wildling (to avoid model convergence issues stemming from complete separation).
We fitted two parallel models from our two seed deposition variables and selected the model with the better AIC.
To catch problems with our model specification, we looked for...
