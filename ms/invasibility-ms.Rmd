...
^2^ Department of Research and Collections, Natural History Museum, University
of Oslo, Oslo, Norway

^3^ Norwegian Institute for Nature Research, Oslo, Norway

^4^ Institute of Urbanism and Landscape, The Oslo School of Architecture and
Design, Oslo, Norway
...
[julienvollering\@gmail.com](mailto:julienvollering@gmail.com){.email}

# Abstract

1.  Plantations of alien conifer species are common in many countries and may
...
2.  We used detailed surveys around 76 plantation stands in Norway to estimate
    ecosystem susceptibility to three groups of commonly planted conifer
    species: *Picea sitchensis* / *Picea* $\times$ *lutzii*, *Picea abies*, and
    *Larix* spp. We created spatially explicit models of wildling abundance as a
    function of ecosystem type, exposure to seed rain, climate, and plantation
    stand characteristics.

3.  Our surveys registered 27 wildlings per hectare for *P. sitchensis* / *P.*
    $\times$ *lutzii* ($\frac{22356}{860}$ ha^-1^), 6 ha^-1^ for *P. abies*
    ($\frac{2245}{396}$ ha^-1^), and 13 ha^-1^ for *Larix*
    spp.($\frac{4213}{319}$ ha^-1^). For all three species groups, we found that
    ...
    height class (≥ 30 cm and ≥ 100 cm), we also found evidence that some
    ecosystems are more conducive to establishment (reaching 30 cm) than growth
    (reaching 100 cm), and vice versa.

4.  Agricultural ecosystems of various kinds were among the ecosystems least
    susceptible to wildlings of all three groups of conifer species.

5.  *Synthesis and applications* We encourage managers to exploit the large
...
introduced species [@faoGlobalForestResources2020]. Alien conifer species may
also be included in some of the vast afforestation programs against climate
change [@brightEvaluatingTerrestrialCarbon2020,
@teururakaunewzealandforestserviceOneBillionTrees2018]. Therefore, it is likely
...
plantation boundaries [@brunduGlobalGuidelinesSustainable2020], but alien
...
spread [@nunezEcologyManagementInvasive2017]. Costly conifer invasions --- like
pines in South Africa, to name one example --- usually stem from forestry
plantations [@esslSelectionCommercialForestry2010;
@mcconnachieEstimatingEffectPlantations2015]. Combined with the expectation of
...
[@burnsSilvicsNorthAmerica1990]. Silviculturalists have long used natural
vegetation composition to forecast the regeneration success of particular
conifer species in particular locations
[@savillPlantationSilvicultureEurope1997], which supports the expectations that:
...
may be masked by copious seed rain [@rougetInferringProcessPattern2003]. Thus,
we distinguish between a location's *susceptibility* (intrinsic favorability for
establishment and growth) and its *vulnerability* [susceptibility multiplied by
exposure to seed rain and other extrinsic factors;
@chaeVulnerabilityResilienceUse2021]. The same distinction exists between
*invasibility* and *level of invasion*, but these terms refer to invader
richness rather than invader abundance [@richardsonPlantInvasionsMerging2006].
...
[@nygaardSpreadIntroducedSitka2017; @nygaardNaturligSpredningAv1999]. Most of
...
*Picea sitchensis* or its hybrid, Lutz spruce *Picea* $\times$ *lutzii*, and
6 500 ha was larch species *Larix* spp.
[@miljodirektoratetUtredningAvForbud2019]. An additional 210 000 ha were planted
with Norway spruce *Picea abies*, which is native but was planted mainly outside
its historical range [@miljodirektoratetUtredningAvForbud2019]. Today,
silvicultural policy in Norway is more restrictive of alien conifers than in
other European countries [@potzelsbergerGrowingNonnativeTrees2020]. Regulations
from 2012 require forest owners to gain approval for new plantations and
implement measures against resulting wildlings. [@ForskriftOmUtsetting2012].
...
semi-natural grasslands, and others [@artsdatabankenFremmedartslista20182018;
@miljodirektoratetUtredningAvForbud2019].
...
invasion science [@pysekScientistsWarningInvasive2020]. To mitigate the negative
impacts of alien conifer plantations, we need to know which ecosystems are most
susceptible to wildlings [@bindewaldSiteSpecificRisk2021;
@potzelsbergerGrowingNonnativeTrees2020]. In the Norwegian context specifically,
...
generalized [@higginsPineInvasionsSouthern1998] --- not across ecosystems nor
across species. Therefore, we must quantify ecosystem susceptibility in a
granular way, by asking: *how likely is establishment and growth of a particular
...
early stages of invasion [@nunezEcologyManagementInvasive2017], but these early
...
[@nathanDispersalKernels2012]. Fourth, we specify our statistical model of the
...
specifications [@arifApplyingStructuralCausal2023].
...
spruce (*Picea sitchensis*) or its fertile hybrid, Lutz spruce (*P.* $\times$
*lutzii*), (2) nineteen with Norway spruce (*Picea abies*), and (3) fifteen with
larch species (*Larix* spp.). Note that Norway spruce is native to Norway, but
the plantations included in this study were located in parts of the country
where its natural distribution is highly restricted. We selected stands using
aerial imagery, aiming for those isolated from conspecific stands. We collected
field data for each stand in one of six field campaigns during the period
2016-2019 [reported in @olsenKartleggingAvKortdistansespredning2016;
@appelgrenKartleggingAvKortdistansespredning2017;
@kyrkjeeideKartleggingAvKortdistansespredning2017;
@appelgrenKartleggingAvKortdistansespredning2018;
@olsenKartleggingAvKortdistansespredning2019;
@sandvenKartleggingAvKortdistansespredning2019].
...
following the Nature in Norway classification system [version 2.0 or 2.1,
@halvorsenNaturNorgeNiN2015; based on the principles summarized in Halvorsen et
al., @halvorsenSystematicsEcodiversityEcoSyst2020]. This system is the national
...
1:5000, which means that any polygon over 250 m^2^ was registered
[@brynVeilederKartleggingAv2015]. Regularly patterned occurrence of more than
...
al. [-@bullockSynthesisEmpiricalPlant2017] the kernel that performed best for
...
Long-distance Dispersal model [WALD, @katulMechanisticAnalyticalModels2005],
following Skarpaas & Shea [-@skarpaasDispersalPatternsDispersal2007]. We
parameterized the model with: site- and season-specific wind vectors retrieved
from meteorological data sets [MET Norway;
@reistadHighresolutionHindcastWind2011; @haakenstad15yearHighResolution2017],
...
gridded point sources, with a density of 0.1 m^-2^ for the first model and
0.01 m^-2^ for the second model (to reduce computation time). Then we applied
...
with \> 0.5 "tree plantation" were excluded from our analyses because some of
...
package [version 1.0, @brooksGlmmTMBBalancesSpeed2017] in R [version 3.6,
@rcoreteamLanguageEnvironmentStatistical2020]. These zero-inflated models regard
...
[@zuurMixedEffectsModels2009]. Preliminary models fitted with a negative
...
[@brooksStatisticalModelingPatterns2019].
...
likelihood [@blasco-morenoWhatDoesZero2019].

To estimate the causal influence of ecosystems on wildling abundance in
accordance with the DAG [@textorRobustCausalInference2016;
@mcelreathHauntedDAGCausal2020], we modeled the count process as dependent on
...
abundance [@westreichTableFallacyPresenting2013]. Climate was represented as
mean annual temperature (Bio1) and precipitation of the coldest quarter (Bio19),
at 30 arcsecond resolution, from CHELSA data
[@kargerClimatologiesHighResolution2017]. We chose these variables because they
showed the strongest correlations with Norwegian vegetation zones and sections,
respectively [@bakkestuenSteplessModelsRegional2008]. Elevation relative to the
...
fixed at 1; @zuurMixedEffectsModels2009]. We expect, for example, that a
...
respectively [@brooksStatisticalModelingPatterns2019]. Subscripts $i$, $j$, and
...
simulated residuals, using the DHARMa package [version 0.4.6,
@hartigDHARMaResidualDiagnostics2020]. We also ran DHARMa's tests for residual
...
larches (table \@ref(tab:estimate-correlation-table)). For instance, our model
estimated that Sitka/Lutz spruce is three times more likely to establish in
"boreal heath" than in "artificial substrate", despite wildling density being
three times lower in "boreal heath" (fig. \@ref(fig:establishment)).
...
table \@ref(tab:model-comparison-table)). Site had a strong influence on
wildling abundance for Sitka/Lutz spruce, such that site variation swamped much
of the variation between ecosystems for this species. The direct effects of
climate on establishment likelihood varied by species and was strongest for
larches (Appendix, tables \@ref(tab:Ps)-\@ref(tab:L)). The direct effects of
...
category or structuring process, to 0--3 orders of magnitude
(fig. \@ref(fig:establishment-hypotheses)). None of the species showed large
differences in establishment likelihood between terrestrial and wetland
ecosystems. At most, larches were three times less likely to establish in
wetlands. Ecosystems structured by destabilizing disturbance tended to show high
establishment likelihoods. However, the association between structuring process
and establishment likelihood was heterogeneous across species.
...
remained mostly unchanged (Appendix, fig. \@ref(fig:establishment-sensitivity)).
...
purely phenomenological model [@bullockAllDispersalFunctions2018]. Nevertheless,
...
[@westreichTableFallacyPresenting2013]. Therefore, we interpret the estimated
...
Norway and its oceanic affinity [@petersonEcologyManagementSitka1997]. For
...
[@tingstadTemperaturePrecipitationBiotic2015], although most of our sites
...
[-@bianchiMethodsPredictingSitka2019] struggled to predict regeneration density
...
@millerHowDisturbanceHistory2021], (2) demographic characteristics of
plantations, potentially related to provenance [especially in cone production;
@taylorDriversPlantInvasion2016], (3) wildling control by property owners, or
...
al.[-@chytryHabitatInvasionsAlien2008] found that alien plants as a group are
...
[@petersonEcologyManagementSitka1997], which might account for why it was
...
[@macekLifeDeathPicea2017]. From this perspective, our estimates of
...
preclude complete containment [@albertLanduseChangeSubalpine2008]. In some cases
...
Norway [@chytryHabitatInvasionsAlien2008]. From a manager's perspective, we
...
[@dovciakSeedRainEnvironmental2008]. On the other hand, experiments might be
...
propagule pressure; @colauttiPropagulePressureNull2006;
@chytrySeparatingHabitatInvasibility2008]. However, many studies quantifying
...
history [@catfordQuantifyingLevelsBiological2012]. We found that accounting for
...
[@richardsonConifersInvasiveAliens2004] and will probably worsen with recent
pushes to increase tree planting worldwide
[@brunduGlobalGuidelinesSustainable2020]. Meanwhile, remotely sensed and
...
at <https://github.com/julienvollering/conifer-plantation-spread>.
...
The WALD model [@katulMechanisticAnalyticalModels2005] takes the form of an
...
function of vegetation height [eq. A4 in
@skarpaasDispersalPatternsDispersal2007]. We parameterized separate WALD models
...
southern Norway [@haakenstad15yearHighResolution2017], or a 10 km resolution
hindcast covering all of Norway [@reistadHighresolutionHindcastWind2011;
@haakenstadNORA10EIRevisedRegional2020]. We used weather station data if the
...
@olsenKartleggingAvKortdistansespredning2019] were used with their point source
...
that of a Sitka spruce monoculture. Likewise, 'mixed forest' stands [e.g. in
@appelgrenKartleggingAvKortdistansespredning2018] were assigned 0.1 times the
seed source point density of a monoculture. Seed source polygons identified as
logged [e.g. in @appelgrenKartleggingAvKortdistansespredning2018] were included
