...
@teururakaunewzealandforestserviceOneBillionTrees2021;
@schubertAdvancesShortfallsApplying]. Therefore, it is likely that alien
conifers will continue to be planted across large areas in coming decades,
sometimes in regions where a given species has not previously occurred.
...
spruce (*Picea sitchensis*) or its fertile hybrid, Lutz spruce (*P.*
$\times$*lutzii*), (2) nineteen with Norway spruce (*Picea abies*), and (3)
fifteen with larch species (*Larix* spp.). Note that Norway spruce is native to
...
period 2016-2019 [reported in @olsenKartleggingAvKortdistansespredning2016;
...
knitr::include_graphics('figures/dagitty-model.svg')
```
...
kbl(cortab, booktabs = TRUE,
    caption = "Correlations between vulnerability (surveyed) and susceptibility (modelled) across ecosystem types, for each species group and wildling class.") %>%
...
```{r susceptibility-Ps, fig.cap="Relative vulnerability (unfilled points) and susceptibility (filled points) of Picea sitchensis / Picea \\times lutzii across ecosystem types, using 'forest' as the reference ecosystem. Zero density is plotted at the lower limit of the x-axis. Relative establishment likelihoods are shown with 95 % confidence intervals. The order of ecosystem types along the y-axis is by susceptibility to wildlings with height $\\geq$ 30 cm."}
knitr::include_graphics('figures/susceptibility/Ps.pdf')
```

```{r susceptibility-Pa, fig.cap="Relative vulnerability (unfilled points) and susceptibility (filled points) of Picea abies across ecosystem types, using 'forest' as the reference ecosystem. Zero density is plotted at the lower limit of the x-axis. Relative establishment likelihoods are shown with 95 % confidence intervals. The order of ecosystem types along the y-axis is by susceptibility to wildlings with height $\\geq$ 30 cm."}
knitr::include_graphics('figures/susceptibility/Pa.pdf')
```

```{r susceptibility-L, fig.cap="Relative vulnerability (unfilled points) and susceptibility (filled points) of Larix spp. across ecosystem types, using 'forest' as the reference ecosystem. Zero density is plotted at the lower limit of the x-axis. Relative establishment likelihoods are shown with 95 % confidence intervals. The order of ecosystem types along the y-axis is by susceptibility to wildlings with height $\\geq$ 30 cm."}
knitr::include_graphics('figures/susceptibility/L.pdf')
```
...
sppcor <- read.csv(here('ms', 'tables', 'corr-between-species.csv'), 
                   row.names = 1, check.names = FALSE)
sppcor <- sppcor %>% filter(species1 != species2) %>% 
  mutate(combo = paste(pmax(species1, species2), pmin(species1, species2))) %>% 
  distinct(hgtclass, combo, .keep_all = TRUE) %>% 
  select(-combo) 

kbl(sppcor, booktabs = TRUE,
    caption = "Correlations in susceptibility of ecosystem types between pairs of species groups, separately for each wildling class.") %>%
  kable_styling(full_width = F) %>% 
...
traits <- tribble(
  ...
  "Picea sitchensis / Picea \\times lutzii", "0.94 m/s", "Oct - Feb", "1, 4",
  "Picea abies", "0.58 m/s", "Nov - May", "2, 4",
  "Larix spp.", "1.0 m/s", "Dec - May", "3, 4")
```
...
kable(traits, longtable = TRUE, booktabs = TRUE, escape = FALSE, 
  ...
  footnote(general = c("1. Harris, A. S. Sitka spruce. in Silvics of North America: 1. Conifers (eds. Burns, R. M. & Honkala, B. H.) vol. 2 513–529 (U.S. Department of Agriculture, Forest Service, 1990).",
                       "2. Sullivan, J. Picea abies. Fire Effects Information System, U.S. Department of Agriculture, Forest Service, Rocky Mountain Research Station,  Fire Sciences Laboratory https://www.fs.usda.gov/database/feis/plants/tree/picabi/all.html (1994)."), 
                       "3. Sullivan, J. Larix decidua. Fire Effects Information System, U.S. Department of Agriculture, Forest Service, Rocky Mountain Research Station,  Fire Sciences Laboratory https://www.fs.usda.gov/database/feis/plants/tree/lardec/all.html (1994).",
...
locs <- read_csv(here('output', 'localities-modified.csv'))
locs <- locs %>% 
  select(year.registered, reference, species, 
         locality, utm33.easting, utm33.northing,
         height.source, age.at.registration, bio01, bio19,
...
locs <- locs %>% 
  mutate(species = case_when(
    species == "P.sitchensis-lutzii" ~ "Picea sitchensis / P. \\times lutzii",
  ...
  rename(`species group` = species, site = locality, easting = utm33.easting, 
         northing = utm33.northing, height = height.source, age = age.at.registration)
# Footnotes
...
typestab <- inner_join(types, pairing) %>%
  ...
  select(type = name, code, `vegetation height` = vegheight)
# Footnotes
names(typestab)[3] <- paste0(names(typestab)[3], footnote_marker_alphabet(1))
```

```{r types-table}
options(knitr.kable.NA = '')
kbl(typestab, longtable = TRUE, booktabs = TRUE, escape = FALSE, 
    caption = "\\label{tab:types-table}Ecosystem types, as defined by major types in the Nature in Norway system (codes). Approximate vegetation height (meters) is used only to parameterize wind turbulence for the WALD seed rain estimates.") %>% 
  kable_styling(latex_options =c("repeat_header"))
```

```{r dispersal-model-comparison-table}
options(knitr.kable.NA = 'NA')
mcomptab <- read.csv(here('ms', 'tables', 'dispmodelcomp.csv'), check.names = FALSE) %>% 
  mutate(`species group` = case_when(
    species == "Ps" ~ "Picea sitchensis / P. \\times lutzii",
    species == "Pa" ~ "Picea abies",
    species == "L" ~ "Larix spp."), .before = 1) %>% 
  select(-species)
kbl(mcomptab, booktabs = TRUE,  escape = FALSE, digits = 0,
...
```{r sensitivity-offset, fig.cap="Shifts in estimated susceptibility when relative seed rain (from the WALD dispersal model) is included in the model as a covariate rather than an offset. Arrows point from the models with offsets to the models with covariates."}
knitr::include_graphics('figures/susceptibility-sensitivity-offset.pdf')
```

```{r sensitivity-seed, fig.cap="Variation in estimated susceptibility to wildlings with height $\\geq$ 100 cm as a function of height assignment to unmeasured individuals. The three estimates for each ecosystem type represent three random seeds used during height assignment."}
knitr::include_graphics('figures/susceptibility-sensitivity-randomseed.pdf')
```
