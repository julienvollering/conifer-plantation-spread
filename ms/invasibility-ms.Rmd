...
For our second analysis, we diagrammed causal relationships among the factors we hypothesized to influence the number of wildlings per cell in a directed acyclical graph (fig. \@ref(fig:DAG)).
...
To estimate how ecosystem type affects wildlings, the directed acyclical graph mandated that we adjust for locality, climate, relative elevation, and seed deposition density in our regression model [@textorRobustCausalInference2016; @mcelreathHauntedDAGCausal2020].
We did not need to adjust for plantation age (with locality in the model), but we included it as a covariate anyway because it could reduce the unexplained variance associated with locality, and because our causal structure would allow us to interpret its coefficient as an (unconfounded) total effect on wildling number [@westreichTableFallacyPresenting2013].
Climate entered the model as mean annual temperature (Bio1) and precipitation of the coldest quarter (Bio19), at 30-arcsecond resolution, from the CHELSA data product [@kargerClimatologiesHighResolution2017].
...
Accordingly, we applied zero-inflated generalized linear models, which model zeros as the mixed product of a binomial process and a conditional count process [@zuurZeroTruncatedZeroInflatedModels2009].
We modeled the binomial process as dependent on plantation age, hypothesizing that younger plantations exhibit more cells without wildlings than would be predicted under a constant rate of colonization due to the stochasticity of the colonization process.
We also modeled the binomial process as dependent on locality, since...
Structural zeros [MEE ref]
Locality was specified as a varying effect.
We used standardized variables (except ecosystem types, which were [0,1]) and fitted models with the glmmTMB package [version 1.0, @brooksGlmmTMBBalancesSpeed2017] in R [version 3.6, @rcoreteamLanguageEnvironmentStatistical2020].
For each species we dropped ecosystem types that did not coincide with a single wildling (to avoid model convergence issues stemming from complete separation) and selected a model by the following procedure.
First, we assessed which of our two variables of seed deposition density better accounted for numbers of wildlings (as judged by AIC of parallel models without zero inflation) and discarded the other.

\begin{equation}
wildings_{ij} \sim ZINB(\pi_{ij}, \mu_{ij}, \phi) \\
logit(\pi_{ij}) = PlantationAge_{ij} + Locality_{ij} \\
log(\mu_{ij}) = SeedDensity_{ij} \cdot PlantationAge_{ij} + Bio1_{ij} + Bio19_{ij} + RelativeElevation_{ij} + Locality_{i} + \sum\limits_{k=1}^K EcosystemType_{ijk} \\
...
The total, unstratified density of wildings was 28/ha for Sitka/Lutz spruce, 6/ha for Norway spruce, 13/ha for larch, and 34/ha for lodgepole pine.

```{r relative establishment, fig.cap="Ecosystem types without wildlings and covering less than 0.5 ha across the species data set are not displayed."}
knitr::include_graphics(here('ms', 'figures', 'relative-establishment.pdf'))
```
