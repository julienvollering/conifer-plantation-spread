...
We used observations of wildlings and ecosystem types collected around 63 conifer plantations, belonging to four groups of alien species (hereafter: "species").
...
locs <- locs %>% 
  ...
  rename(plantation = species) %>% 
  mutate(locality = paste(toupper(substring(locality, 1, 1)), 
                          substring(locality, 2), sep = "")) %>%
  rename(easting = utm33.easting, northing = utm33.northing) %>% 
  mutate(easting = round(easting), northing = round(northing)) %>% 
  mutate(bio01 = round(bio01/10, digits = 2), bio19 = round(bio19/10, digits = 2))

```

```{r localities-table}
kable(select(locs, -height.interpolated, -age.interpolated)) %>% 
  kable_styling(bootstrap_options = c("striped", "condensed")) %>% 
  footnote(general = "Easting and northing are given for UTM zone 33N, bio01 is given in ºC, and bio19 is given in cm.")
```
