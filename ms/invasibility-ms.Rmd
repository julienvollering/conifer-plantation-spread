...
Accordingly, we applied zero-inflated generalized linear models, which model zeros as the mixed product of a binomial process and a count process [@zuurZeroTruncatedZeroInflatedModels2009].
...
\begin{equation}
wildings_{i} \sim ZINB(\pi_{i}, \mu_{i}, k) \\
logit(\pi_{i}) = PlantationAge_{i} + Locality_{i} \\
log(\mu_{i}) = SeedDensity_{i} x PlantationAge_{i} + Bio1_{i} + Bio19_{i} + RelativeElevation_{i} + Locality_{i} + \sigma... \\
Locality_{i} \sim Normal(0, \sigma^2) \\
(\#eq:statmod)
\end{equation}

where $\pi$ is the probability of a zero, while $\mu$ and $k$ are the mean and dispersion parameter of the negative binomial distribution, respectively [eq. 11.21 in @zuurZeroTruncatedZeroInflatedModels2009].  
