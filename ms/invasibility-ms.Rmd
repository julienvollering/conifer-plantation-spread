...
msumms <- readRDS(here('ms', 'tables', 'modelsummaries.rds'))
msumms <- msumms %>% 
  filter(is.na(hgtclass) | (hgtclass == 2 & randomseed == 2))

for (i in nrow(msumms)) {
  kable(msumms$tidymodel[[i]][,-1], longtable = TRUE, booktabs = TRUE, align = 'lrrrr', 
        caption = paste0(
          '\\label{tab:',
          paste0(msumms$species[[i]],msumms$hgtclass[[i]]),
          '}Model summary for ',
          paste(msumms$speciesname[[i]], msumms$hgtclassname[[i]], sep = ', '), 
          ". The conditional submodel is glmmTMB's genpois (Generalized Poisson) family with dispersion parameter $\\phi^{2}$ = ",
          msumms$sigma[[i]])) %>%
    add_header_above(c(" " = 1, "Fixed effects" = 2, "Random effects" = 2)) %>% 
    pack_rows(index = c("Conditional model" = sum(msumms$tidymodel[[i]][,1]=="Conditional Model"),
                        "Zero-inflation model" = sum(msumms$tidymodel[[i]][,1]=="Zero-inflation model")))
}
```
