...
To test whether higher-level characteristics of ecosystem types can be used to generalize patterns, we aggregated ecosystem types by their category (terrestrial or wetland) or structuring process (anthrogenic, natural or no disturbance), as defined in the Nature in Norway system (Appendix [A3]).
...
The described recruitment patterns of the three other species are difficult to evaluate ecosystem invasibility against.
...
From this perspective, our estimates of establishment probability measure the density of suitable microsites in a given ecosystem type.
...
In the results from our naive approach, differences in raw wildling density between ecosystem types are partly attributable to differences in their invasibility, and partly to contingent properties of our sample, including its composition of sites and the spatial arrangement of ecosystem types around seed sources at those sites.
For example, if a particular ecosystem type tended to occur close to plantations in our sample, it would likely show higher wildling density than other ecosystem types, even if the species were equally likely to establish there.
In our modeling approach, accounting for seed rain, climate, and site amplified the differences between ecosystem types. 
This implies that, in our sample, confounding factors partly counteracted differences in ecosystem establishment --- e.g. vulnerable ecosystems may have been concentrated at sites otherwise predicted to show limited spread.
Therefore, direct observations of relative abundances misguide intuition about relative vulnerability. 
For example, the density of Sitka spruce wildlings was about equal in boreal heath and sand dunes, but our estimates suggest that establishment likelihood is actually about 100 times larger in boreal heath. 
For management: do not underestimate the influence of plantation surroundings --- site selection matters!
