...
The sample comprised thirty-six plantations of Sitka/Lutz spruce, twelve plantations of larch, nine plantations of Norway spruce, and six plantations of lodgepole pine.
...
Because conspecific plantations were sometimes located nearby the plantation of interest, we considered all plantations within a 1 km radius to be potential seed sources.
...
Specifically, we selected from Bullock et al. [-@bullockSynthesisEmpiricalPlant2017] the kernel that performed best for 5-15 m tall trees with wind-adapted seeds (an Exponential Power function).
Fifty-seven of the 63 plantations in our data set matched this height range better than a taller height range with a different kernel.
For this first model, polygons of seed sources were transformed into point sources by...
...
where $H$ is the seed release height, $U$ is the mean horizontal wind velocity between $H$ and the ground, $F$ is the terminal velocity of the seed, and $\sigma$ is a wind turbulence parameter that we estimated as a function of vegetation height [see Appendix in @skarpaasDispersalPatternsDispersal2007 for details].
We parameterized separate WALD models for 20-degree sectors around each seed source, to make seed dispersal anisotropic (directional).
In each sector we estimated vegetation height based on the composition of mapped ecosystem types (Appendix [A2]), and randomly sampled 100 wind velocities from all measurements in the direction of the sector.
The seed probability density in the sector was obtained by summing the estimates of the 100 resulting WALD kernels. 
Then we combined sectors, weighing the sectors by the frequency distribution of corresponding wind directions during the species' dispersal season (Appendix [A3]).
For this second model, polygons of seed sources were transformed into point sources by...

We used wind data from...
