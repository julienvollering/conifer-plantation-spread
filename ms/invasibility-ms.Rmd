...
The motivation for this approach was descriptive; we summarized the state of wildling establishment from alien conifer plantations under prevailing conditions.
In the results from this analysis, variation in the density of wildlings is partly attributable to differences between species and between ecosystem types, and partly to contingent properties of our sample, such as the spatial distribution of ecosystem types around plantations.
...
For our analyses of establishment frequency, we rasterized wildling occurences and ecosystem types to the same 10 m grid as the seed dispersal models (fig. \@ref(fig:locality-example)). 
...
Area covered by mosaic polygons was evenly divided among the constituent types.

We excluded the "tree plantation" ecosystem type from our analyses because wildlings occurring within this type were not registered in all field campaigns.
...
We diagrammed causal relationships among factors influencing the number of wildlings per cell with a directed acyclical graph (Appendix [A5]).
Its structure mandated that --- to estimate the effect of ecosystem type on wildlings --- our model adjust for climate, relative elevation, seed deposition density, and locality [ @textorRobustCausalInference2016; @mcelreathHauntedDAGCausal2020].
...
```{r DAG, echo=FALSE, fig.cap="A caption", out.width = '100%'}
