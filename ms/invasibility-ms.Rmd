...
Plantations of wind-dispersed, alien conifers present a golden opportunity to assess ecosystem invasibility (to these species), because we can estimate ecosystem exposure (seed rain) directly, rather than by proxy [@catfordQuantifyingLevelsBiological2012]. 
...
We parameterized the model with: site- and season-specific wind vectors retrieved from meterological data sets, wind turbulence estimated from local ecosystem type composition, seed release height based on plantation height, and species-specific seed terminal velocities from literature.
...
In the DAG, the unmeasured, proximate causes of wildling abundance --- establishment likelihood and seed rain --- are decendants of variables that we could observe or model.
...
To check how the covarates in our models swayed estimates of establishment likelihood, we calculated the observed density of wildlings in each ecosystem type (abundance/area).
...
The underestimation implies that confounding variables counteracted differences in ecosystem establishment in our sample --- for instance that vulnerable ecosystems were concentrated at sites predicted to show limited spread.
...
Simulataneously, we randomly sampled 100 wind velocities in the direction of the sector during the species' dispersal season.
