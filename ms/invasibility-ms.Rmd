...
Chytrý et al.[-@chytryHabitatInvasionsAlien2008] found that alien plants as a group were consistently found at low rates in mires and heaths, and high rates in arable, man-made, and coastal ecosystem types.
The conifers we studied --- perhaps with the exception of lodgepole pine --- did not conform to these broader trends in ecosystem invasibility.
However, our estimates of ecosystem invasibility do align with some findings from previous studies of species-specific recruitment patterns.
...
This illustrates the trouble with deriving predictions for management units such as ecosystem types from generalized statements about species autecology; should we expect few wildlings in 'open shallow-soil ground' because it is dry, or many because it provides ample light and reduced competition that Sitka spruce seedlings need [@petersonEcologyManagementSitka1997]?
Moreover, microsites strongly regulate seedling mortality [@macekLifeDeathPicea2017], so ecosystem types that would seem inhospitible at first glance may actually contain many suitable microsites for establishment.
From this perspective, our estimates of establishment probability measure the density of suitable microsites, which may be treated as constant even if their fine-scale spatial distribution is not.
Among the species we examined, the establishment of lodgepole pine has been studied most, and our results seem consistent with this literature. 
...
Our results suggest that differences between ecosystem types deserve careful consideration when managing wildling spread.
This knowledge can be applied in at least two ways.
First, as a preventative measure, we recommend that new plantations be sited where surrounded by high proportions of ecosystem types with low establishment likelihood.  
...
Long distance dispersal might preclude complete containment [@albertLanduseChangeSubalpine2008], but dramatic reductions in the number of wildlings are likely. 
In some cases it may also be desireable to alter ecosystem types adjacent to existing plantations to prevent (further) spread, for example by intensifying mowing to promote the appearance of 'agriculturally improved grassland'.
Second, as a reactive measure, we recommend that monitoring and control be targeted to highly vulnerable ecosystem types.
Applying this strategy to ecosystem types that are both vulnerable and rare, like 'open active landslide', is especially likely to be cost-efficient. 

The establishment patterns we quantify for four species are likely to hold, more or less, in other countries [@chytryHabitatInvasionsAlien2008].
From a manager's perspective, we expect that the ecosystem types we report may be successfully translated to equivalent types in similar classification systems, because Nature in Norway classification is rule-based and aims for observer neutrality. 
...
An observational study like ours is most directly relevant for managing long-lived species like conifers, because its temporal scope can be longer than is typically practical for experimental studies.
It measures long-term survival --- the quantity of interest for management --- under a wide range of natural conditions experienced by the wildlings.
...
For example, Sitka spruce may be more likely to germinate in disturbed soil [@vikaneInvasionCallunaHeath2013], but less likely to survive there [@petersonEcologyManagementSitka1997].
The precision of invasibility estimates will also be higher for widely introduced species, like those used in plantations, because a more representative sample of wildling spread will be measurable.
Additional insight could be gleaned from a study like ours if wildling growth rates were quantified in some way.
Field observations of interwhorl distances suggest that individuals grew taller at very different rates, depending on their location.  
Abundant but slow colonization may be of lesser concern than sparse but fast colonization, depending on management objectives.
...
In the results from the naive approach, variation in the density of wildlings is partly attributable to differences between species and between ecosystem types, and partly to contingent properties of our sample, such as explained (climatic or ecosystem) and unexplained variation between sites.
...
For management: do not underestimate the influence of plantation surroundings --- site selection matters! Direct observations of relative abundances misguide intuition about relative vulnerability. For example, the density of Sitka spruce wildlings was about equal in boreal heath and sand dunes, but our estimates suggest that establishment likelihood is actually about 100 times larger in boreal heath. For management the difference between a ten-fold gap and a 1000-fold gap matters --- and may draw more attention to plantation surroundings.
...
With only six sites in two distinct groups, differences attributed to climate may have arisen by other mechanisms. Climate may have masked correlated drivers (e.g. systematic differences in ecosystem composition) 
