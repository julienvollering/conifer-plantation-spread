...
E in SUCCES: To manage an invading conifer, should we assess its invasiveness or where the invasibility of the land?

Here, we examine how the surroundings of a forestry plantation affects spread of individuals by quantifying the relative probability of establishment in different nature types. Probability of establishment is by definition normalized with respect to the number of arriving propagules, and is therefore independent of the seed production and seed dispersal of the plantation. It is a quantity that results from physiological processes of seed survival, germination and growth, and therefore is expected to vary between species. 
...
- We hypothesize that nature types characterized by irregular disturbance events will show the highest susceptibility, that nature types without canopy cover will show high susceptibility, that nature types with canopy cover will show low susceptibility, and that nature types with regular human intervention will show lowest susceptibility. We also hypothesis that these differences will be greatest for species which are fast-growing and shade-intolerant.

3. How important is distance from a plantation compared to invasibility in determining the colonization rate of a particular area? I.e. how far does a highly susceptible nature type have to be from a plantation to have the same rate of spread as an insusceptible nature type? NOTE: We are assessing the affect of distance mechanistically, not observationally. I.e. we are not learning about the effect of distance in our model --- we have assumed it acts only through propagule presure, and do not treat it as a variable in the model.

- We hypothesize that the differences in invasibility of nature types are outweighed by proximity to seed sources on a sub-hectometer (100 m) scale.

4. Are characteristics of the plantation or characteristics of the surroundings the more important determinants of spread? Is variation in abundance of wildlings greater between sites (primarily plantation differences) or within sites (primarily nature type differences)? 

- We hypothesize that abundance of wildlings varies more between sites than between nature types, and that this variation reflects primarily the seed production of the plantation (unmeasured).

# Introduction

Plantations are abundant and extensive source populations, and their surroundings are vulnerable to spread.
Managers are interested in limiting or eliminating spread from plantations.
...
However, some underlying environmental characteristics may also affect the seed shadow; for example, lower elevations may trap more seeds.
