...
Spread in *arithmetically calculated* establishment density vs. spread in *modeled* establishment (among ecosystems for which both values are estimated):

```{r}
filter(dat, is.na(locality)) %>% 
  filter(!(modeled == FALSE & fit == 0)) %>% 
  group_by(species, modeled) %>% 
  summarize(spread = sd(log10(fit)))
```
