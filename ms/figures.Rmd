...
```{r}
ggsave(g123, filename = 'localities-map.svg', path = here('ms', 'figures'),
...
plotwet <- dwet %>% 
  mutate(category = fct_relevel(category, 'terrestrial', '*wetland*'))
...
plotdisturb <- ddisturb %>% 
  mutate(structuring = fct_relevel(structuring, 'no<br>disturbance',
                                   '**natural<br>disturbance**',
                                   '**anthropogenic<br>disturbance**'))
