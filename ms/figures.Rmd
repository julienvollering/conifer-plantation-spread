...
layoutheights <- filter(plotting, randomseed == 1, hgtclass == 2) %>% 
  group_by(species) %>% 
  summarize(n = n()) %>% 
...
```{r}
  ...
  # geom_linerange(data = filter(d, modeled == TRUE, is.na(locality), is.na(randomseed)),
  #                mapping = aes(xmin = ci.l, xmax = ci.u), color = col, size = 1) +
  # geom_point(data = filter(d, modeled == TRUE, is.na(locality), is.na(randomseed)),
  #            color = col, size = csize) +
