...
types <- read_csv(here('data', 'types.csv')) %>% 
  ...
  mutate(name = case_when(
    category == 'terrestrial' & is.na(structuring) ~ name,
    category == 'terrestrial' & !is.na(structuring) ~ paste0(paste0('**',name),'**'),
    category == 'wetland' & is.na(structuring) ~ paste0(paste0('*',name),'*'),
    category == 'wetland' & !is.na(structuring) ~ paste0(paste0('***',name),'***'))
  )
reflvl <- "T4"
...
arith <- bind_rows(results$arithmetic, .id = "species") %>% 
  filter(!(n == 0 & daa < 5)) %>% 
  filter(nin != "plantation forest") %>%  #TODO
  rename(type = nin) %>% 
...
```{r}
plot <- dat %>% 
  mutate(species = fct_relevel(species, c('P.sitchensis-lutzii', 
                                          'P.abies', 
                                          'Larix', 
                                          'P.contorta')))
