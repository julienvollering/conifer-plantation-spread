...
surveyedarea <- plotting %>% 
  ...
  mutate(hgtclass = NA_real_)
plotting <- plotting %>% 
  left_join(perspecies, by = join_by(species, hgtclass)) %>% 
  left_join(surveyedarea, by = join_by(nin, hgtclass))
```
...
ggplot(d, aes(x = density.per.ha, y = name, fill = species)) + 
  geom_blank() +
  ...
  geom_linerange(aes(xmin=species.per.ha, xmax=density.per.ha, color = species), 
                 ...
                 linetype = 5, alpha = 0.5) +
  geom_point(aes(color = species), size = csize, shape = 21,
             position = position_dodge(width = 0.5)) +
  geom_text(aes(x = 110, label = ifelse(is.na(total.ha), "", paste(round(total.ha), "ha"))),
            hjust = "right") +
  ...
  scale_fill_manual(values = alpha(c("P.sitchensis-lutzii" = CBdark2.3class[1],
                                     "P.abies" = CBdark2.3class[2],
                                     "Larix" = CBdark2.3class[3]), 1)) +
  facet_grid(cols = vars(hgtclassname)) +
  labs(title = "<span style = 'color:#d95f02;'>*Picea sitchensis* / *Picea* \u00D7 *lutzii*</span>, <span style = 'color:#7570b3;'>*Picea abies*</span>, <span style = 'color:#1b9e77;'>*Larix* spp.</span>",
       x = "surveyed wildling abundance (ha<sup>-1</sup>)") +
  guides(shape = guide_legend(ncol = 2, title = element_blank(), reverse = TRUE)) +
  topbottomtheme +
  theme(legend.position="none")

ggsave(filename= 'abundance.svg', 
...
```{r}
ggsave(g123, filename= 'abundance-sensitivity-randomseed.svg', 
       path = here('ms', 'figures'),
...
d <- filter(plotting, species == "P.sitchensis-lutzii", randomseed == 2 | is.na(randomseed)) %>% 
  ...
  mutate(name = fct_reorder(name, fit, first))
g <- ggplot(d, aes(x = fit, y = name, shape = pointshape)) + 
  geom_blank() +
  geom_hline(yintercept = which(levels(d$name) == 'forest'), linetype = "dotted") +
  ...
  coord_cartesian(xlim = c(1e-4, 1e2)) +
  facet_grid(cols = vars(hgtclassname)) +
  labs(title = '*Picea sitchensis* / *Picea* \u00D7 *lutzii*',
...
ggsave(g, filename= 'Ps.svg', 
       ...
       scale = 1.4, width = 210, height = layoutheights[1]*3+20, units = 'mm', dpi = 300)
```
...
ggsave(g, filename= 'Pa.svg', 
       path = here('ms', 'figures', 'susceptibility'),
       scale = 1.4, width = 210, height = layoutheights[2]*3+20, units = 'mm', dpi = 300)
```
...
ggsave(g, filename= 'L.svg', 
       path = here('ms', 'figures', 'susceptibility'),
       scale = 1.4, width = 210, height = layoutheights[3]*3+20, units = 'mm', dpi = 300)
```
