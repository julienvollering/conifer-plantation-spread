...
g1 <- ggplot(d, aes(x = fit, y = name)) + 
  ...
  geom_linerange(mapping = aes(xmin = ci.l, xmax = ci.u), color = coltop, size = 1) +
  geom_point(color = coltop, size = csize) +
...
g2 <- ggplot(d, aes(x = fit, y = name)) + 
  geom_blank() +
  geom_hline(yintercept = which(levels(d$name) == 'forest'), linetype = "dotted") +
  geom_linerange(mapping = aes(xmin = ci.l, xmax = ci.u), color = colmid, size = 1) +
  geom_point(color = colmid, size = csize) +
  scale_x_log10(labels = scales::trans_format("log10", scales::math_format(10^.x))) +
...
g3 <- ggplot(d, aes(x = fit, y = name)) + 
  geom_blank() +
  geom_hline(yintercept = which(levels(d$name) == 'forest'), linetype = "dotted") +
  geom_linerange(mapping = aes(xmin = ci.l, xmax = ci.u), color = colbot, size = 1) +
  geom_point(color = colbot, size = csize) +
  scale_x_log10(labels = scales::trans_format("log10", scales::math_format(10^.x))) +
...
g1 <- ggplot(d, aes(x = fit, y = name, group = randomseed)) + 
  geom_blank() +
  geom_hline(yintercept = which(levels(d$name) == 'forest'), linetype = "dotted") +
  geom_linerange(mapping = aes(xmin = ci.l, xmax = ci.u), color = coltop, size = 0.5,
                 position = position_dodge(width = 0.5)) +
  geom_point(color = coltop, size = 1, position = position_dodge(width = 0.5)) +
  scale_x_log10(labels = scales::trans_format("log10", scales::math_format(10^.x))) +
  coord_cartesian(xlim = c(1e-4, 1e2)) +
  facet_grid(cols = vars(hgtclassname)) +
  toptheme
...
g2 <- ggplot(d, aes(x = fit, y = name, group = randomseed)) + 
  geom_blank() +
  geom_hline(yintercept = which(levels(d$name) == 'forest'), linetype = "dotted") +
  geom_linerange(mapping = aes(xmin = ci.l, xmax = ci.u), color = colmid, size = 0.5,
                 position = position_dodge(width = 0.5)) +
  geom_point(color = colmid, size = 1, position = position_dodge(width = 0.5)) +
  scale_x_log10(labels = scales::trans_format("log10", scales::math_format(10^.x))) +
  coord_cartesian(xlim = c(1e-5, 1e1)) +
  facet_grid(cols = vars(hgtclassname)) +
  middletheme
...
g3 <- ggplot(d, aes(x = fit, y = name, group = randomseed)) + 
  geom_blank() +
  geom_hline(yintercept = which(levels(d$name) == 'forest'), linetype = "dotted") +
  geom_linerange(mapping = aes(xmin = ci.l, xmax = ci.u), color = colbot, size = 0.5, 
                 position = position_dodge(width = 0.5)) +
  geom_point(color = colbot, size = 1, position = position_dodge(width = 0.5)) +
  scale_x_log10(labels = scales::trans_format("log10", scales::math_format(10^.x))) +
  coord_cartesian(xlim = c(1e-3, 1e3)) +
  facet_grid(cols = vars(hgtclassname)) +
  labs(x = 'relative establishment likelihood') +
...
temp <- results %>% 
  ...
  unnest(preds)
ggplot(temp, aes(fit, locality, color = nin)) + geom_point() + coord_cartesian(xlim = c(0,10))
...
g1 <- ggplot(d, aes(x = fit, y = name)) + 
  ...
  geom_point(data = filter(d, modeled == FALSE), 
             color = coltop, pch = 1, size = csize+1) +
  geom_linerange(data = filter(d, modeled, is.na(locality)),
                 mapping = aes(xmin = ci.l, xmax = ci.u), color = coltop, size = 1) +
  geom_point(data = filter(d, modeled, is.na(locality)), color = coltop, size = csize) +
  scale_x_log10(labels = scales::trans_format("log10", scales::math_format(10^.x))) +
  coord_cartesian(xlim = c(1e-4, 1e2)) +
  toptheme
...
g2 <- ggplot(d, aes(x = fit, y = name)) + 
  ...
  geom_point(data = filter(d, modeled, !is.na(locality)),
             ...
             position = position_jitter(width = 0, height = 0.25)) +
  geom_point(data = filter(d, modeled == FALSE), 
             color = colmid, pch = 1, size = csize+1) +
  geom_linerange(data = filter(d, modeled, is.na(locality)),
                 mapping = aes(xmin = ci.l, xmax = ci.u), color = colmid, size = 1) +
  geom_point(data = filter(d, modeled, is.na(locality)), color = colmid, size = csize) +
  scale_x_log10(labels = scales::trans_format("log10", scales::math_format(10^.x))) +
  coord_cartesian(xlim = c(1e-5, 1e1)) +
  middletheme
...
g3 <- ggplot(d, aes(x = fit, y = name)) + 
  ...
  geom_point(data = filter(d, modeled, !is.na(locality)),
             ...
             position = position_jitter(width = 0, height = 0.25)) +
  geom_point(data = filter(d, modeled == FALSE), 
             color = colbot, pch = 1, size = csize+1) +
  geom_linerange(data = filter(d, modeled, is.na(locality)),
                 mapping = aes(xmin = ci.l, xmax = ci.u), color = colbot, size = 1) +
  geom_point(data = filter(d, modeled, is.na(locality)), color = colbot, size = csize) +
  scale_x_log10(labels = scales::trans_format("log10", scales::math_format(10^.x))) +
  coord_cartesian(xlim = c(1e-3, 1e3)) +
  labs(x = 'relative establishment likelihood') +
  bottomtheme
...
g2 <- ggplot(d, aes(x = fit, y = name)) + 
  ...
  geom_hline(yintercept = which(levels(d$name) == 'forest'), linetype = "dotted") +
  geom_segment(data = filter(d, diff != 0), 
               mapping = aes(yend = name, x = fit, xend = fit.end), color = colmid, 
               size = 2, lineend = 'round', linejoin = 'mitre',
               arrow = arrow(length = unit(0.001, "npc"), angle = 45, type = "closed")) +
  geom_point(data = filter(d, diff == 0),
             mapping = aes(x = fit), color = colmid, size = 2) +  
  scale_x_log10(labels = scales::trans_format("log10", scales::math_format(10^.x))) +
  coord_cartesian(xlim = c(1e-5, 1e1)) +
  middletheme
...
g3 <- ggplot(d, aes(x = fit, y = name)) + 
  ...
  geom_hline(yintercept = which(levels(d$name) == 'forest'), linetype = "dotted") +
  geom_segment(data = filter(d, diff != 0), 
               mapping = aes(yend = name, x = fit, xend = fit.end), color = colbot, 
               size = 2, lineend = 'round', linejoin = 'mitre',
               arrow = arrow(length = unit(0.001, "npc"), angle = 45, type = "closed")) +
  geom_point(data = filter(d, diff == 0),
             mapping = aes(x = fit), color = colbot, size = 2) +  
  scale_x_log10(labels = scales::trans_format("log10", scales::math_format(10^.x))) +
  coord_cartesian(xlim = c(1e-3, 1e3)) +
  labs(x = 'relative establishment likelihood') +
  bottomtheme
